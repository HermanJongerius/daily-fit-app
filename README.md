...
**Huidige versie: 1.10.0** (zie ook `version` in `package.json` — deze twee horen altijd gelijk te zijn).
...
- **1.10.0** (7 sep 2026) — Grote nieuwe functionaliteit, op verzoek: het plaatje dat op het voortgangsscherm onthuld wordt, is bij een volledig afgeronde trainingsweek (7 van de 7 dagen) vervangen door een echte video met waardevolle informatie over bewegen. Het principe blijft hetzelfde (dag 1 t/m 6 blijft de bestaande plaatsvervangende illustratie zichtbaar, onthuld vakje voor vakje), maar op dag 7 van een perfecte week verschijnt in plaats van het complete plaatje nu een video, op volgorde: de eerste perfecte week ontgrendelt Video 1, de tweede Video 2, enzovoort, voor iedereen in dezelfde volgorde. Nieuw beheerscherm "Video's" waar deze video's geüpload en beheerd worden — zelfde manier van uploaden als bij de dagelijkse oefenvideo's (rechtstreeks naar hetzelfde Cloudflare Stream-account), maar als eigen, losse lijst. Is de lijst op (meer perfecte weken dan geüploade video's), dan blijft de laatst toegevoegde video zichtbaar totdat er een nieuwe wordt toegevoegd.
- **1.9.0** (7 sep 2026) — Op verzoek: de flow na de training is vereenvoudigd. Het voortgangsscherm (plaatje + tekst) schakelt niet meer automatisch na 10 seconden door — de gebruiker gaat nu zelf, bewust, via de knop "Verder" naar het volgende scherm. Dat scherm (op "Vandaag") toont voortaan overal dezelfde afsluitende tekst "Tot morgen! Je bent nu klaar." — zowel direct na het klikken op "Verder" als wanneer iemand later diezelfde dag opnieuw inlogt (dit vervangt de tekst "Knap gedaan!" uit versie 1.8.2).
- **1.8.2** (7 sep 2026) — Op verzoek: de tekst op het scherm "Vandaag" die verschijnt zodra de dagelijkse oefening al gedaan is, is aangepast van "Je hebt vandaag al bewogen!" naar "Knap gedaan!" — dat past beter bij het moment vlak na het afronden van de training zelf. De regel eronder ("Morgen staan de … oefeningen klaar.") blijft ongewijzigd.
- **1.8.1** (7 sep 2026) — Op verzoek: het lettertype van de veiligheidsdisclaimer op de video-pagina is vergroot (van 12px naar 15px), in verband met de doelgroep.
- **1.8.0** (7 sep 2026) — Kleine veiligheidsdisclaimer toegevoegd boven de video op de oefenpagina, in verband met aansprakelijkheid: "Voor uw veiligheid: overleg bij twijfel met uw huisarts, stop bij pijn of duizeligheid, en oefen op eigen tempo in een veilige, opgeruimde ruimte." Let op: dit is inhoudelijk opgesteld, geen juridisch getoetste tekst — laat de exacte formulering het liefst nog even meekijken door een jurist of verzekeraar voordat dit definitief live gaat.
- **1.7.1** (7 sep 2026) — Op verzoek: het voortgangsscherm (het beloningsscherm na de training) springt nu na 10 seconden vanzelf terug naar "Vandaag" in plaats van na 5 seconden — zo is er wat meer tijd om het plaatje en de tekst rustig te bekijken. De "Verder"-knop om direct door te gaan staat er nog steeds gewoon bij.
- **1.7.0** (4 sep 2026) — Nieuwe knop "Exporteren naar Excel" boven het gebruikersoverzicht van de beheerder: levert een Excel-bestand (.xlsx) op met van alle deelnemers de naam, het mobiele nummer, de betaaldatum + status (actief/verlopen), de aanmelddatum, en de trainingsvoortgang (aantal keer getraind, mogelijke traindagen, percentage) — handig om buiten de app overzicht te houden, te printen of te delen.
- **1.6.2** (4 sep 2026) — Echte bugfix: na een échte (dus niet de testknop in ontwikkelmodus) video ging het scherm altijd terug naar "Vandaag", in plaats van naar het nieuwe voortgangsscherm (beloningsscherm, versie 1.5.0). Oorzaak: de server stuurde na het registreren van een training keurig door naar het voortgangsscherm, maar de code in de browser negeerde dat en ging altijd hardcoded naar "Vandaag" terug. Hierdoor kreeg iedereen die met een echte video traint het beloningsscherm nooit te zien (de dev-knop in de testomgeving gebruikt een andere weg en werkte toevallig wel, waardoor dit niet eerder opviel). Dit is nu opgelost: de browser gaat naar het adres dat de server aangeeft. Er is een automatische controle toegevoegd die dit specifieke pad voortaan meetest.
- **1.6.1** (4 sep 2026) — Echte bugfix voor een tijdzone-fout: de server (op Railway, en ook hier tijdens het bouwen) houdt intern de tijd in UTC bij, terwijl Nederland daar het hele jaar 1 à 2 uur op voorloopt. Daardoor kon een training die vlak na Nederlandse middernacht werd gedaan (bijvoorbeeld 00:30 's nachts) per ongeluk onder de vorige dag geregistreerd worden, met als zichtbaar gevolg dat op het scherm "Vandaag" het verkeerde dagbolletje oplichtte (bijv. de D van donderdag terwijl het al vrijdag was). Alle plekken waar de app bepaalt "welke dag is het vandaag" — de dagelijkse oefening, de dagbolletjes, het voortgangsscherm, de verloopdatum-controle en het beheerscherm — gaan nu uit van de Nederlandse klok (Europe/Amsterdam) in plaats van de klok van de server zelf.
- **1.6.0** (4 sep 2026) — In het gebruikersoverzicht van de beheerder staat nu bij elke senior direct te zien hoevaak diegene in totaal getraind heeft, en welk percentage dat is van het aantal dagen dat hij/zij had kúnnen trainen (gerekend vanaf de dag van aanmelden tot en met vandaag) — bijvoorbeeld "6x getraind · 60% (van 10 dagen sinds aanmelden)".
- **1.5.0** (4 sep 2026) — Nieuw voortgangsscherm na elke training: een plaatje met 7 vakjes erover, waarvan er na elke training één verdwijnt. De cyclus van 7 dagen loopt per deelnemer vanaf de datum dat zijn/haar account is aangemaakt (dus niet gekoppeld aan maandag-zondag). Op dag 1 t/m 6 van de cyclus staat een korte aanmoediging ("Nog X vakjes te gaan"); op dag 7 verschijnt één van drie eindteksten, afhankelijk van hoeveel van de 7 trainingen zijn gelukt (7 = compleet plaatje + felicitatie, 4-6 = "trots op je", 0-3 = "lekker bezig, probeer een vast moment in te plannen"). Na 5 seconden gaat het scherm vanzelf terug naar "Vandaag" (met een knop ernaast om direct door te gaan). Het plaatje achter de vakjes is nu nog een eigen gemaakte illustratie als plaatsvervanger — zodra jij een echte foto aanlevert, vervang ik die met één regel code.
...
- Trainingsstatistiek per deelnemer (aantal keer getraind + percentage) in het gebruikersoverzicht, met een knop om alle deelnemers (naam, telefoonnummer, betaalstatus, voortgang) te exporteren naar een Excel-bestand.
